...
`inbound-close` (`uv run inbound-close --repo OWNER/NAME --issue N --ready M`) closes a superseded inbound (`bifrost:in` that is not `work:ready` and not `bifrost:out`) after a new ready issue exists. Comments a pointer to `#M`, then closes N. Fail closed. Does not apply `work:ready`. Mill untouched. Not on the Fala monitor path.
