"""Read-only JSON of outbound GitHub items that must not ship yet.

Open issues and PRs labeled bifrost:out without verdict:pass (including
verdict:hold / reject / needs-scout, or no verdict). Heimdall plus mill
...
ATOM = "observe-outbound"
FALA_REACTION = "github.observe_outbound"
...
def keep_outbound(labels: list[str]) -> bool:
    """Keep bifrost:out that must not ship yet — missing verdict:pass."""
    return OUTBOUND in labels and PASS not in labels


def as_outbound(repo: str, item: dict[str, Any], *, kind: str) -> dict[str, Any]:
    return {
        "repo": repo,
...
def survey_repo(repo: str, *, run: GhFn) -> list[dict[str, Any]]:
    ...
    for issue in list_issues(repo, run=run):
        if keep_outbound(issue["labels"]):
            out.append(as_outbound(repo, issue, kind="issue"))
    for pull in list_pulls(repo, run=run):
        pull.pop("_head", None)
        if keep_outbound(pull["labels"]):
            out.append(as_outbound(repo, pull, kind="pull"))
    return out
...
def observe_outbound(
    *,
    ...
    """List outbound items that must not ship. Fail closed on gh errors."""
    run = run or gh
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="observe-outbound",
        description=(
            "Print a JSON envelope of open GitHub items that claim outbound "
            "(bifrost:out) but must not ship yet (missing verdict:pass). "
            "Read-only."
        ),
    )
    parser.add_argument(
    ...
    return emit_exit(observe_outbound(heimdall=args.heimdall))
